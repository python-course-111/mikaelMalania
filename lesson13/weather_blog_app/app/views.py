...
def get_html_content(request):
    ...
    html_content = session.get(
        f'https://www.google.com/search?q=weather+{city}').text
    return html_content
...
def weather(request):
    ...
    if 'city' in request.GET:
        ...
        coordinates = httpx.get(
            f'https://geocode.maps.co/search?q={requested_city}')
        # getCoordinates = httpx
        coordinates = coordinates.json()

        latitue = coordinates[0]['lat']

        longitude = coordinates[0]['lon']

        weather_at_location = httpx.get(
            f"https://api.open-meteo.com/v1/forecast?latitude={latitue}&longitude={longitude}&current=temperature_2m,wind_speed_10m&hourly=temperature_2m")

        weather_at_location = weather_at_location.json()

        current_weather = weather_at_location["current"]["temperature_2m"]

        return render(request, 'weather.html', {'result': current_weather})
